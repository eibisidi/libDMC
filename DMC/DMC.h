...
返回值：正确：返回 ERR_NoError；
错误：返回相关错误码。
注意：由于ECM底层原因，只有当某个轴回零完成后，另一个轴才能启动回零
*/
DMC1000_API DWORD WINAPI d1000_home_move(short axis);
...
返回值：返回值：输入口状态： 1 On 0 Off
*/
DMC1000_API DWORD WINAPI d1000_in_bit(short ioslave_idx, short BitNo);
